...
Notify("NO KEY! HURRAY! Game Took "..string.format("%0.2f", diffTimeGame).." Seconds To Load")
...
local function loadSettings()
    if(isfile and readfile) then
        if(isfile("FREEHugeGames.json")) then
            data = HttpService:JSONDecode(readfile("FREEHugeGames.json"))
            recurseTable(data)
            ...
            if not (data["saveVersion"] == getgenv().settings["saveVersion"]) then
                writefile("FREEHugeGames.json", (HttpService:JSONEncode(getgenv().settings)))
...
function getAreas()
    allAreas = {}
    worlds = game:GetService("ReplicatedStorage").Framework.Shared["1 | Directory"].Areas:GetChildren()
...
function createUILibrary()
    --Start Functions
    local UILib = loadstring(game:HttpGet("https://raw.githubusercontent.com/FREEHugeGames/PSX/main/UILib.lua"))()
    local UILib = UILib.CreateWindow("Huge Games", (scriptVer))

    --Tabs
    local TabEst = UILib:Tab("Essentials")
    local TabFarming = UILib:Tab("Farming")
    local TabEgg = UILib:Tab("Auto Eggs")
    ...
    discordSection:Label("discord.gg/HUGEGames")
    discordSection:Button("Copy Invite Link", function()
        setclipboard("discord.gg/HUGEGames")
    end)
    discordSection:Label("==] Credits [==")
    discordSection:Label("Natt#6965 // Owner/Developer")
    discordSection:Label("Twekky#00001 // Co-Owner/Developer")
    discordSection:Label("4lve#0001 // Ex-Owner/Developer")

  
    local SectionSpoof = TabFree:Section("Spoofing")
    local SectionDiscordStat = TabFree:Section("Stat Trackers")
    local SectionPetTeams = TabFree:Section("Pet Teams")
    local SectionHoverboard = TabFree:Section("Hoverboard")
    local SectionMerchantHop = TabFree:Section("Merchant Hop")
    local SectionMastery = TabFree:Section("Auto Mastery")
    SectionMerchantHop:DropDown("Merchant Hopper Mode", {"Rainbow Hellish Axolotl", "Buy Tier 3"}, function(option)
            if option == "Rainbow Pixel Wolf" then
                getgenv().mode = 2
            else
                getgenv().mode = 1
            end
        ...
        SectionSpoof:DropDown("Select Player", plrList, function(plr)
            selctedPlayer = plr
        end)
        SectionSpoof:Button("Spoof Selected Player", function()
        ...
        SectionSpoof:Button("Un-Spoof Player", function()
            pcall(function()
        ...
        SectionSpoof:Button("Show Twitter Name", function()
        ...
        SectionDiscordStat:TextBox("<-- Enter Discord ID and push Enter", "", function(id)
            getgenv().discordId = id
        end)
        SectionDiscordStat:Toggle("Stat Trackers", false, function(isToggled)
            if isToggled then
                getgenv().sendReq = true
                spawn(function()
                    loadstring(game:HttpGet("https://raw.githubusercontent.com/FREEHugeGames/PSX/main/API.lua"))()
                end) 
            else
    ...
    --Auto Redeem Gift
  	...
  	redeemSection:Toggle("Auto Redeem Free Gift", getgenv().settings.game.autofreegift, function(bool)
    ...
    redeemSection:Toggle("[NEW!] Auto Claim Loot Bags", getgenv().settings.game.autoCollectBags, function(bool)
    ...
    mainEggSection:Label("Inventory will auto open/close every 5 mins")
    ...
    --Tripple Hatch
    mainEggSection:Toggle("Tripple Hatch (Requires Gamepass)", getgenv().settings.autoEggs.trippleHatch, function(isToggled)
        if isToggled then
    ...
    ColletionSection:Toggle("Tripple Hatch (Requires Gamepass)", getgenv().settings.colection.trippleHatch, function(isToggled)
                ...
                table.insert(allConnected, Lib.Network.Fired("Open Egg"):Connect(function(egg, openTable)
                    if tempSettings.colection.deleteEnabled then
                        ...
                        for i,v in ipairs(openTable) do
                            ...
                            if (Lib.Directory.Pets[v.id]["rarity"] == "Exclusive") then
                                continue
                            end
                            matched = false
    ...
    ColletionSection:Toggle("Open Eggs That Is'nt In Index", false, function(isToggled)
    ...
    Notify("Script Took "..(string.format("%0.2f", diffTime)).." Seconds To Fully Load")
